...
const registerValidate = () => {
    return [
        ...
        body('password').trim().isLength({min : 4}).withMessage('Şifre en az 6 karakterden oluşmalıdır'),
        body('password').trim().isLength({max : 20}).withMessage('Şifre en fazla 20 karakterden oluşmalıdır'),

        body('username').trim().isLength({max : 6}).withMessage('Kullanıcı adı en az 6 karakterden oluşmalıdır'),
        body('username').trim().isLength({max : 14}).withMessage('Kullanıcı en fazla 14 karakterden oluşmalıdır'),

        body('full_name').trim().isLength({max : 6}).withMessage('Adınız ve soyadınız en az 6 karakterden oluşmalıdır'),
